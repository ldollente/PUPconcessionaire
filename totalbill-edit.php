...
if(isset($_POST['update']))
{
    ...
    // Call the stored procedure to update the total_bill table
    $stmt = $con->prepare("CALL update_total_bill(?, ?)");
    $stmt->bind_param("ss", $eid, $due_date);
    $stmt->execute();
    
    if($stmt->affected_rows > 0)
    {
      $update_query = "UPDATE total_bill SET total_bill = $total WHERE bill_id = '$eid'";
      mysqli_query($con, $update_query);
        echo "<script>alert('Update successfully!');</script>";
        echo "<script>document.location = 'totalbill-index.php';</script>";
    }
    else
    {
...
<html lang="en">
  ...
  <div class="topnav" id="myTopnav">
    ...
    <h2>Total Bill Information</h2>

                <?php 
                    $eid = $_GET['editid'];
                    $sql = mysqli_query($con, "SELECT * FROM  `total_bill` WHERE bill_id = '$eid'");
                ...
                ?>
                    ...
                    <input type="text" class="form-control" name="total_bill" value="<?php echo $row['total_bill'] = $row['s_bill'] + $row['e_bill'] + $row['w_bill']; ?>" readonly>
              ...
              <div class='form_buttons' >
                    ...
                    <button>
                    <a class='cancel' href="totalbill-index.php" class="btn btn-danger">Cancel</a>
                    </button>
                </div>
            </form>
        </div>
