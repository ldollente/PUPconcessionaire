...
if(isset($_POST['update'])) {
  ...
  $stmt = $con->prepare("CALL update_assigned_space(?, ?)");
  $stmt->bind_param("ss", $space_id, $eid);
  $stmt->execute();
  
  if($stmt) {
      echo "<script>alert('New Space successfully added!');</script>";
      echo "<script>document.location = 'assignedspace-index.php';</script>";
  }
  else {
            ...
            <h2>Assigned Space Information</h2>
                <div class='form_group' class="row mb-3">
                    ...
                    <div class="col">
                        ...
                        <input type="text" class="form-control" name="space_id" >
                    </div>
